...
if (!$conn){
 ...
 // getting the customer using sessions   
$cusID = $_SESSION['$cID'];
    ?>
<html lang="en">
    <meta charset="utf-8">
...
<link href="https://fonts.googleapis.com/css2?family=Alkatra&family=PT+Serif&family=Quicksand:wght@300&display=swap" rel="stylesheet">
    ...
    <body>
        ...
        <div style="text-align:center; text-decoration:underline; color:  #46E14E;">
            <h4>Below are your Bookings as a space owner</h4>
        </div>    
        <div style="text-align:center; text-decoration:none;  line-height: 35px;"><?php 
           // retirving the space owners booking from the booking table using the customer ID
            $sql = "select * from booking where customerID = $cusID limit 10";
 $result = mysqli_query($conn, $sql);
            $count = mysqli_num_rows($result);
             if (mysqli_num_rows($result) > 0){
                 while($row = mysqli_fetch_assoc($result)){                
                     
        echo "Space ID:". $row['ownerID']. ", " ." Booking ID: " . $row["bookingID"].   ",  " . " Address: ". $row["spaceaddress"]. ", ".  " Postcode: ". $row["spacepostcode"]. ", ". " Space Type: ". $row["spaceType"]. ", ". " Date:". $row["date"]. ", ". " Time: ". $row["time"]. ", ". " Amount: "."£". $row["amount"]. "<br>";   
                     }}
            ?>
